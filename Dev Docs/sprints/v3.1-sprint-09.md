...
- [x] Add ONNX/quantized embedding backends in `retikon_core/embeddings/`.
- [x] Add routing hooks/interfaces in `retikon_core/query_engine/` (no GPU tiers enabled).

## Scope checklist (Pro)
- [x] Add GPU query service profile/entrypoint in `gcp_adapter/`.
- [ ] Deploy GPU services via `infrastructure/terraform/`.
- Deferred to v3.2 Sprint 00 (carryover closeout).

## Scope checklist (Docs)
- [x] Add SLO load tests for text-only vs multimodal queries in
...
- [x] Core: `tests/core/test_embedding_backends.py`
- [x] Core: `tests/core/test_query_routing.py`
- [x] Pro: `tests/pro/test_query_tiers.py` (new)

## Deliverables
- [ ] CPU-optimized (ONNX/quantized) and GPU query tiers available in Pro.
- Deferred to v3.2 Sprint 00 (carryover closeout).
- [x] Documented routing rules and performance baselines.
