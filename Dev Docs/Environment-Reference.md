...
- `RERANK_MODEL_NAME` (defaults to `cross-encoder/ms-marco-MiniLM-L-6-v2`)
- `RERANK_BACKEND=hf|onnx|quantized` (defaults to `hf`)
- `RERANK_TOP_N` (defaults to `20`)
- `RERANK_BATCH_SIZE` (defaults to `8`)
- `RERANK_QUERY_MAX_TOKENS` (defaults to `32`)
- `RERANK_DOC_MAX_TOKENS` (defaults to `128`)
- `RERANK_MIN_CANDIDATES` (defaults to `2`)
- `RERANK_MAX_TOTAL_CHARS` (defaults to `6000`)
- `RERANK_SKIP_SCORE_GAP` (defaults to `1.0`; disables confidence-gap skip unless overridden)
- `RERANK_SKIP_MIN_SCORE` (defaults to `0.7`)
- `RERANK_TIMEOUT_S` (defaults to `2.0`)
- `RERANK_ONNX_MODEL_PATH` (optional override for ONNX/quantized reranker path)
- `MODEL_INFERENCE_RERANK_TIMEOUT_S` (overrides global inference timeout for rerank)
...
- `QUERY_FUSION_RRF_K` (defaults to `60`)
- `QUERY_FUSION_WEIGHTS` (optional CSV/JSON; defaults to built-in v1 weights)
- `QUERY_FUSION_WEIGHT_VERSION` (defaults to `v1`)
